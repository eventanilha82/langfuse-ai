...
const ChatSidebar = memo(function ChatSidebar({
  ...
  const handleThemeToggle = useCallback(() => {
    ...
    const nextSettings = { ...uiSettings, darkMode: uiSettings.darkMode !== true };
    try {
      window.localStorage?.setItem("uiSettings", JSON.stringify(nextSettings));
    } catch {
      // The visual toggle should still work if storage is unavailable.
    }
    window.dispatchEvent(new CustomEvent("uiSettingsChanged", { detail: nextSettings }));
  }, [onToggleDarkMode, uiSettings]);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {chatHistoryLength === 0 && attachmentCount === 0 && (
          <motion.div
            ...
            style={{ overflow: "hidden", minHeight: "100px", flexShrink: 0 }}
          >
            <Box id="chat-sidebar-message" sx={{ minHeight: "100px" }}>
              <TypingEffect text="Oi," speed={50}>
                {(displayedText) => (
                  <Typography
                    variant="h3"
                    sx={{
      ...
      {/* Bottom section - Recent conversations + theme toggle */}
      <Box
      ...
      >
        ...
        {/* Theme icon */}
        <Box sx={{ position: "fixed", left: 32, bottom: 32, zIndex: 120 }}>
          <IconButton
            onClick={handleThemeToggle}
            title={isDarkBg ? "Modo claro" : "Modo escuro"}
          ...
          >
            {isDarkBg ? <Sun size={19} /> : <Moon size={19} />}
          </IconButton>
        </Box>
      </Box>
    </Box>
